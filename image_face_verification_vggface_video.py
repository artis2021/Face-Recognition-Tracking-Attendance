...
def detect_extract_face(image_to_detect):
    ...
    print('There are {} no of faces in this frame '.format(len(all_face_locations)))
...
while True:
    ...
    #image_to_classify = cv2.cvtColor(image_to_classify, cv2.COLOR_BGR2RGB)
    ...
    if sample_faces[1] is not None:   
        ...
        #the face that will be verfied against
        video_face = sample_faces_embeddings[1]
    
        # Verify against the known photographs using cosine distance
        cosine_distance = str(cosine(modi_face_1, video_face))

        cv2.putText(image_to_classify,cosine_distance,(50,50),cv2.FONT_HERSHEY_PLAIN,2.5,(0,255,0),2)
